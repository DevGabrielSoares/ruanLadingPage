...
export default function TestimonialsCarousel(){
  return (
    <>
      <style jsx global>{`
        .custom-testimonial-buttons { display: none; }
        @media (min-width: 768px) { .custom-testimonial-buttons { display: block; } }
      ...
      <section id="depoimentos" className="bg-slate-50 dark:bg-gray-900 py-16 sm:py-24 overflow-hidden">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
              ...
              <motion.p 
                className="mt-2 text-lg leading-8 text-slate-600 dark:text-slate-400"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
              ...
              >
                  {testimonialsData.map((testimonial) => (
                      <SwiperSlide key={testimonial.id}>
                          <motion.div
                            className="flex h-full flex-col bg-white dark:bg-slate-800 p-8 rounded-lg shadow-lg border border-transparent dark:border-slate-700"
